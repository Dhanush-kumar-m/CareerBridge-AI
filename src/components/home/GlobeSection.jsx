...
// Procedural granular gold texture — no image asset, no licensing concerns.
function useGoldTexture() {
  return useMemo(() => {
    ...
    for (let i = 0; i < 9000; i++) {
      ...
      ctx.fillStyle = `rgba(${shade},${(shade * 0.65) | 0},${(shade * 0.2) | 0},${Math.random() * 0.5 + 0.15})`;
      ctx.beginPath();
    ...
    <group ref={groupRef}>
      <mesh>
        ...
        {texture && (
          <meshStandardMaterial
            ...
            metalness={0.75}
            roughness={0.38}
            emissive={new THREE.Color(0x1a0f02)}
      ...
      {nodePositions.map((pos, i) => (
        <mesh key={NODES[i].name} position={pos}>
          ...
          <meshBasicMaterial color="#FFD199" />
        </mesh>
      ))}
    ...
    <section className={styles.globeSection}>
      ...
      <div className={styles.canvasWrap}>
        <Canvas camera={{ position: [0, 0.4, 7.2], fov: 45 }}>
          <ambientLight color="#332211" intensity={0.6} />
          <pointLight color="#ffd9a0" intensity={2.2} position={[4, 3, 4]} />
          <pointLight color="#ff8c00" intensity={0.5} position={[-4, -2, -3]} />
          <Globe />
        </Canvas>
      </div>
    </section>
