...
const ScrollStack = ({
  ...
  const updateCardTransforms = useCallback((scrollTopVal) => {
    ...
    const endElementTop = endElementPositionRef.current;

    // Find current top card index based on scroll position
    let topCardIndex = 0;
    ...
    cardsRef.current.forEach((card, i) => {
      ...
      // Blur & Opacity fade calculations for cards pushed to background
      let blur = 0;
      let opacity = 1;
      ...
      if (i < topCardIndex) {
        ...
        opacity = Math.max(0.1, 1 - depth * 0.35); // Fade out card as it stacks behind
      }
      ...
      const hasChanged =
        ...
        Math.abs(lastTransform.opacity - newTransform.opacity) > 0.05;
  ...
  const setupLenis = useCallback(() => {
    if (useWindowScroll) {
      // Listen directly to native window scroll instead of creating a nested Lenis instance
  ...
  useLayoutEffect(() => {
    ...
    const recalculatePositions = () => {
      ...
      // Temporarily clear transforms to measure original positions
      const originalTransforms = cards.map(c => c.style.transform);
      const originalFilters = cards.map(c => c.style.filter);
      const originalOpacities = cards.map(c => c.style.opacity);
      ...
      if (endElement) {
        if (useWindowScroll) {
          ...
          endElementPositionRef.current = rect.top + window.scrollY;
        } else {
          endElementPositionRef.current = endElement.offsetTop;
        }
      }

      // Restore transforms
      cards.forEach((c, i) => {
        c.style.transform = originalTransforms[i];
        c.style.filter = originalFilters[i];
        c.style.opacity = originalOpacities[i];
      });
    };
