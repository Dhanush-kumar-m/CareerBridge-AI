...
export default function VerbalPage() {
  ...
  useEffect(() => {
    if (!loadingProgress) {
      const questionsOfLevel = aptitudeData.verbal.filter(
        (q) => q.difficulty.toLowerCase() === level.toLowerCase()
      );
      const unsolved = questionsOfLevel.filter(
        (q) => !solvedList.includes(`verbal_${q.id}`)
      );
    ...
    if (currentQuestionIndex < totalQuestions - 1) {
      ...
      if (moduleId) {
        ...
        if (nextModule) {
          ...
          setIsModuleCompleted(true);
          setPopupMessage(`Module ${moduleId} completed! All verbal practice complete! 🥳`);
          redirectTimeoutRef.current = setTimeout(() => {
            setPopupMessage("");
            setIsModuleCompleted(false);
            setHasAnswered(false);
        ...
        setTimeout(() => {
          ...
          if (level === "Easy") {
            ...
            router.push("/aptitude/reasoning");
          }
        }, 2500);
      }
    }
  ...
  return (
    <div className="aptitude-page">
      ...
      <div className="page-header">
        ...
        <p>
          Improve your communication, grammar, vocabulary and reading
          comprehension skills for placements.
        </p>
      </div>
        ...
        {totalQuestions === 0 ? (
          <div style={{
            ...
            <p style={{ color: "#94a3b8", fontSize: "1.05rem", marginBottom: "24px" }}>
              Congratulations! You have completed every question in the {level} section of Verbal Ability.
            </p>
            <div style={{ display: "flex", justifyContent: "center", gap: "12px" }}>
              <button
                onClick={() => handleResetLevel("verbal")}
                className="solve-btn"
                style={{
              ...
              {level !== "Hard" ? (
                ...
                <button
                  onClick={() => router.push("/aptitude/reasoning")}
                  className="solve-btn"
                  style={{
                    background: "linear-gradient(135deg, #2563eb, #7c3aed)",
                    color: "#fff",
