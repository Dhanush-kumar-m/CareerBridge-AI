...
export default function DataInterpretationPage() {
  ...
  const handleLevelChange = (lvl) => {
    ...
    const unsolved = questionsOfLevel.filter(
      (q) => !solvedList.includes(`dataInterpretation_${q.id}`)
    );
    setSessionQuestions(unsolved);
  };
  ...
  const handleAnswerSubmit = (isCorrect) => {
    ...
    if (isCorrect) {
      ...
      if (currentQuestion) {
        const key = `dataInterpretation_${currentQuestion.id}`;
      ...
      if (moduleId) {
        ...
        setTimeout(() => {
          ...
          if (level === "Easy") {
            ...
            const unsolved = questionsOfLevel.filter(
              (q) => !updatedList.includes(`dataInterpretation_${q.id}`)
            );
            setSessionQuestions(unsolved);
            ...
            router.push("/aptitude/abstract-reasoning");
          }
        }, 2500);
      }
    }
  ...
  return (
    <div className="aptitude-page">
      ...
      <div className="page-header">
        ...
        <p>
          Analyze charts, tables, bar diagrams, and pie charts to draw logical conclusions.
        </p>
      </div>
        ...
        {totalQuestions === 0 ? (
          <div style={{
            ...
            <p style={{ color: "#94a3b8", fontSize: "1.05rem", marginBottom: "24px" }}>
              Congratulations! You have completed every question in the {level} section of Data Interpretation.
            </p>
            <div style={{ display: "flex", justifyContent: "center", gap: "12px" }}>
              <button
                onClick={() => handleResetLevel("dataInterpretation")}
                className="solve-btn"
                style={{
              ...
              {level !== "Hard" ? (
                ...
                <button
                  onClick={() => router.push("/aptitude/abstract-reasoning")}
                  className="solve-btn"
                  style={{
                    background: "linear-gradient(135deg, #2563eb, #7c3aed)",
                    color: "#fff",
