...
export default function AbstractReasoningPage() {
  ...
  const handleLevelChange = (lvl) => {
    ...
    const unsolved = questionsOfLevel.filter(
      (q) => !solvedList.includes(`abstractReasoning_${q.id}`)
    );
    setSessionQuestions(unsolved);
  };
  ...
  const handleAnswerSubmit = (isCorrect) => {
    ...
    if (isCorrect) {
      ...
      if (currentQuestion) {
        const key = `abstractReasoning_${currentQuestion.id}`;
      ...
      setTimeout(() => {
        ...
        if (currentQuestionIndex < totalQuestions - 1) {
          ...
          if (level === "Easy") {
            ...
            const unsolved = questionsOfLevel.filter(
              (q) => !updatedList.includes(`abstractReasoning_${q.id}`)
            );
            setSessionQuestions(unsolved);
  ...
  return (
    <div className="aptitude-page">
      ...
      <div className="page-header">
        ...
        <p>
          Identify patterns, rules, and trends in shapes, sequences, and diagrams.
        </p>
      </div>
        ...
        {totalQuestions === 0 ? (
          <div style={{
            ...
            <p style={{ color: "#94a3b8", fontSize: "1.05rem", marginBottom: "24px" }}>
              Congratulations! You have completed every question in the {level} section of Abstract Reasoning.
            </p>
            <div style={{ display: "flex", justifyContent: "center", gap: "12px" }}>
              <button
                onClick={() => handleResetLevel("abstractReasoning")}
                className="solve-btn"
                style={{
