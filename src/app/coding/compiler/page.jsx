...
export default function CompilerPage() {
  ...
  const activeCodeKey = `${question.id}-${language.toLowerCase()}`;
  const editorCode = questionCodes[activeCodeKey] || "";
  ...
  // Reset compiler status and output when question or language changes
  useEffect(() => {
    setPassed(false);
  ...
  const handleCodeChange = (newCode) => {
    ...
    // Auto-save code for this specific question and language
    const key = `${question.id}-${language.toLowerCase()}`;
  ...
  const runCode = () => {
    ...
    const userCode = editorCode || getDefaultCodeForQuestion(question, language);
